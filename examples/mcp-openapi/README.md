...
```bash
cargo run -- -f examples/mcp-openapi/config.yaml
```

In addition to the [basic](../mcp-basic) setup, we have added a new target of type `openapi`:

```yaml
name: openapi
openapi:
  schema:
    file: ./examples/mcp-openapi/openapi.json
  host: localhost:8080
```
