...
helm upgrade -i semantic-router oci://ghcr.io/vllm-project/charts/semantic-router \
  ...
  -f examples/llm-semantic-routing/k8s/cost-based/semantic-router-values.yaml \
...
```bash
kubectl apply \
  -f examples/llm-semantic-routing/k8s/cost-based/agentgateway-routing.yaml
...
```bash
kubectl apply \
  -f examples/llm-semantic-routing/k8s/cost-based/force-auto.yaml
```
