...
For a gateway-native browser login flow, use the [traffic-oidc](../traffic-oidc/README.md) example instead.
...
docker compose -f examples/traffic-oauth2-proxy/docker-compose.yaml up
```
...
```bash
cargo run -- -f examples/traffic-oauth2-proxy/config.yaml
```

Start an upstream application on port 8080:
