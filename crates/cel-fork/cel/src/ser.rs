		...
		T: ?Sized + Serialize,
	{
		self.map.insert(
			self.next_key.clone().ok_or_else(|| {
				SerializationError::InvalidKey("serialize_value called before serialize_key".to_string())
			})?,
			value.serialize(Serializer)?,
		);
		Ok(())
		...
		match key {
			...
			Duration::NANOS_FIELD => {
				...
				self.nanos = val.try_into().map_err(|_| {
					SerializationError::SerdeError("timestamp struct nanos field is invalid".to_owned())
				})?;
				Ok(())
			},
...
impl ser::Serializer for TimeSerializer {
	...
	fn serialize_str(self, v: &str) -> Result<Self::Ok> {
		...
		Ok(
			v.parse::<chrono::DateTime<FixedOffset>>()
...
mod tests {
	...
	fn test_json_data_conversion() {
		...
		let program = Program::compile("expected.all(key, serialized[key] == expected[key])").unwrap();
