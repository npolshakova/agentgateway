				...
				Some((idx, c2)) => {
					let byte: u8 = match c2 {
						'x' => {
							...
							.iter()
							.collect();
							u8::from_str_radix(&hex, 16).map_err(|_| ParseSequenceError::InvalidEscape {
								escape: hex,
								index: idx,
								string: s.to_string(),
							})?
						...
						n if ('0'..='3').contains(&n) => {
							...
							.iter()
							.collect();
							u8::from_str_radix(&octal, 8).map_err(|_| ParseSequenceError::InvalidEscape {
								escape: octal,
								index: idx,
								string: s.to_string(),
							})?
			...
			match chars.next() {
				...
				Some((idx, c2)) => {
					...
					let value = match c2 {
						...
						n if ('0'..='3').contains(&n) => {
							parse_unicode_oct(&n, &mut chars).map_err(|x| ParseSequenceError::InvalidUnicode {
								source: x.clone(),
								index: idx,
								string: String::from(s),
							})?
