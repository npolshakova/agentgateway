...
export function RawConfigEditor({ invalid, onChange, onSave, value }: RawConfigEditorProps) {
	...
	function mountEditor(editor: Monaco.editor.IStandaloneCodeEditor, monaco: typeof Monaco) {
	...
	return (
		<div
			className={
				invalid ? 'editor-wrap raw-config-editor invalid' : 'editor-wrap raw-config-editor'
			}
		>
			<Editor
...
async function validateEditorText(
	...
	try {
		...
		if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
			return [marker(monaco, model, 1, 1, 'Configuration must be a YAML object.')];
		}
		const errors = await getGatewayConfigValidationErrors(parsed as GatewayConfig);
		...
		const mark = (err as { mark?: { line?: number; column?: number }; message?: string }).mark;
		return [
			marker(
				monaco,
				model,
...
function marker(
	...
	const safeColumn = Math.min(Math.max(column, 1), model.getLineMaxColumn(safeLine));
	return {
