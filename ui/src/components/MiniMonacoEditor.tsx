...
export function MiniMonacoEditor(props: MiniEditorProps) {
	const monacoLanguage = props.language === 'cel' ? celLanguage : props.language;
	return (
		<div className={miniEditorClassName(props.className, props.invalid)}>
			<Editor
				...
				options={{
					...
					quickSuggestions:
						props.language === 'cel' ? { other: true, comments: false, strings: false } : false,
...
function miniEditorClassName(className: string | undefined, invalid: boolean | undefined) {
	return ['editor-wrap', 'mini', 'mini-monaco-editor', className, invalid ? 'invalid' : null]
		.filter(Boolean)
		.join(' ');
}
