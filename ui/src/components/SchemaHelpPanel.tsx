import { YamlTextBlock } from '@/components/Primitives';
...
function schemaNodeLines(
	...
	if (variantNodes.length > 1 && !resolved.properties) {
		...
		const firstObject = variantNodes.find(variant => resolveDisplayNode(variant, help).properties);
		if (firstObject)
			lines.push(
	...
	for (const [key, child] of entries) {
		...
		const childDescription = cleanDescription(child.description ?? childResolved.description);
		if (childDescription) pushComment(lines, childDescription, indent);
		const meta = [schemaTypeLabel(childResolved), required.has(key) ? 'required' : '']
			.filter(Boolean)
			.join(', ');
		...
		const objectNode = objectVariant ? resolveDisplayNode(objectVariant, help) : childResolved;
		if (objectNode.properties && options.maxDepth > 1) {
			...
			lines.push(`${spaces(indent)}${key}: ${placeholderForSchema(childResolved)}`);
		}
		lines.push('');
	}
