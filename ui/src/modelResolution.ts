...
export function selectedConfiguredModelName(concreteModel: string, models: LlmModel[]) {
	...
	const wildcard = models
		...
		.sort((left, right) => {
			const specificity =
				wildcardSpecificity(right.model.name) - wildcardSpecificity(left.model.name);
			return specificity || left.index - right.index;
		})[0]?.model;
	return wildcard?.name ?? models[0]?.name ?? '';
}

export function concreteModelName(configuredModelName: string, specificModel: string) {
