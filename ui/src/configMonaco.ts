...
export function registerConfigYamlSchema(monaco: typeof Monaco, path: string, schema: unknown) {
...
export function installYamlKeySuggest(editor: Monaco.editor.IStandaloneCodeEditor) {
...
export function toMonacoYamlSchema(schema: unknown): unknown {
	...
	for (const [key, value] of Object.entries(source)) {
		...
		if (key === 'format' && typeof value === 'string' && !knownJsonSchemaFormats.has(value)) {
			continue;
		}
		...
		provideCompletionItems(model, position) {
			...
			return {
				suggestions: Object.entries(properties)
					...
					.map(([name, property]) => {
						const resolved = editableObjectSchema(property) ?? resolveSchema(property);
...
function schemaForModel(model: Monaco.editor.ITextModel) {
	...
	if (uri === `file:///${rawConfigModelPath}`) return monacoConfigSchema as Record<string, unknown>;
...
function objectSchemaAtPath(
	...
	let current: Record<string, unknown> | undefined = editableObjectSchema(schema);
...
function editableObjectSchema(
	...
	if (typeof schema.$ref === 'string') {
		...
		const resolved = schema.$ref.startsWith('#/definitions/')
			? getByPath(monacoConfigSchema as Record<string, unknown>, schema.$ref.slice(2).split('/'))
			: undefined;
	...
	if (Array.isArray(schema.allOf)) {
		return schema.allOf.reduce<Record<string, unknown>>((merged, candidate) => {
			const resolved = editableObjectSchema(candidate as Record<string, unknown>, seenRefs);
	...
	const merged = branches.reduce<Record<string, unknown>>(
		(current, branch) => {
			const resolved = editableObjectSchema(branch as Record<string, unknown>, seenRefs);
			if (!resolved || schemaTypeValues(resolved).includes('null') || !isStructuredSchema(resolved))
				return current;
...
function resolveSchema(
	...
	if (anyOf) {
		...
		return resolveSchema(branch as Record<string, unknown> | undefined, seenRefs);
	}
	if (Array.isArray(schema.allOf)) {
		return schema.allOf.reduce<Record<string, unknown>>((merged, candidate) => {
	...
	if (branches) {
		if (shouldCollapseBranches(branches, root)) {
			...
			if (branch && typeof branch === 'object' && !Array.isArray(branch)) {
				return {
					...normalizeEditorSchemaNode(branch as Record<string, unknown>, root, expandingRefs),
					...copySchemaAnnotations(schema)
				};
			}
		}
		...
		next.properties = Object.fromEntries(
			Object.entries(next.properties).map(([key, value]) => [
				...
				value && typeof value === 'object' && !Array.isArray(value)
					? normalizeEditorSchemaNode(value as Record<string, unknown>, root, expandingRefs)
					: value
			])
		);
	}
...
function selectEditableBranch(
	...
	return nonNull.find(candidate => !isConditionalPolicyBranch(candidate, root)) ?? nonNull[0];
}

function shouldCollapseBranches(branches: unknown[], root: Record<string, unknown>) {
	const nonNull = branches.filter(
		candidate => resolveBranchCandidate(candidate, root)?.type !== 'null'
	);
	return (
		nonNull.length === 1 || nonNull.some(candidate => isConditionalPolicyBranch(candidate, root))
	);
}

function isConditionalPolicyBranch(candidate: unknown, root: Record<string, unknown>) {
...
function resolveBranchCandidate(candidate: unknown, root: Record<string, unknown>) {
	if (!candidate || typeof candidate !== 'object' || Array.isArray(candidate)) return undefined;
	const schema = candidate as Record<string, unknown>;
	if (typeof schema.$ref === 'string') return resolveSchemaReference(root, schema.$ref);
	return schema;
}
