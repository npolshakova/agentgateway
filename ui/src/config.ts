...
const completionLogExpression = 'llm.completion.map(c, {"role":"assistant", "content": c})';
...
function hasLegacyUiPromptCompletionMappings(add: Record<string, string> | undefined) {
	return (
...
export function upsertVirtualModel(
	...
	llm.virtualModels ??= [];
	const index =
		llm.virtualModels?.findIndex(item => item.name === (previousName ?? model.name)) ?? -1;
...
export function upsertLlmProvider(
	...
	llm.providers ??= [];
	const index =
		llm.providers?.findIndex(item => item.name === (previousName ?? provider.name)) ?? -1;
...
export function isDatabaseConfigResource(
	...
	return (resources ?? []).some(resource => resource.kind === kind && resource.id === id);
}
...
export function upsertVirtualKey(config: GatewayConfig, key: VirtualApiKey, previousKey?: string) {
	...
	const index = policy.keys.findIndex(item => keyValue(item) === (previousKey ?? keyValue(key)));
	if (index >= 0) {
...
export function promptCompletionLoggingEnabled(config: GatewayConfig | undefined) {
	const accessLog = config?.frontendPolicies?.accessLog as AccessLogConfig | undefined;
	...
	return [accessLog?.database?.add, accessLog?.add].some(hasLegacyUiPromptCompletionMappings);
}

export function setPromptCompletionLogging(config: GatewayConfig, enabled: boolean) {
...
export function setUiLogAttributeExpressions(
	...
	if (Object.keys(attributes).length === 0) delete config.config.standardAttributes;
...
export function configWarnings(
	...
	if (duplicateIds.length > 0) {
		warnings.push(`Duplicate model IDs: ${Array.from(new Set(duplicateIds)).join(', ')}.`);
	}
	const apiPolicy = (effectiveLlm?.policies.apiKey ?? config.llm?.policies?.apiKey) as
		| LlmApiKeyPolicy
		| null
		| undefined;
