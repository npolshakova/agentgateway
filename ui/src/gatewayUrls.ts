...
export function gatewayEndpoint(port: number, path = '', scheme = currentWindowScheme()) {
...
function playgroundEndpoint(
	...
	if (gateway?.port !== undefined) {
		return {
			baseUrl: gatewayEndpoint(gateway.port, path, gateway.scheme ?? currentWindowScheme()),
			sameOrigin: false
		};
	}
	return {
...
function gatewayRefsOverlap(config: GatewayConfig, leftRefs: string[], rightRefs: string[]) {
	const left = new Set(expandGatewayRefs(config, leftRefs).map(item => item.ref));
	return expandGatewayRefs(config, rightRefs).some(item => left.has(item.ref));
}
...
function firstGatewayEndpoint(
	...
	return refs?.flatMap(ref => expandGatewayRef(config, ref)).find(item => item.port !== undefined);
}
