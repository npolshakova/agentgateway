...
import { useConfigDumpMode, useRawGatewayConfig, useUpdateConfig } from '@/hooks';
import {
	ReadonlyModeBanner,
...
function TrafficListenersEditorPage() {
	...
	const linkedListener = linkedListenerEditor(trafficDrawer, config.data?.binds ?? []);
			...
			<PageHeader
				...
				actions={
					<div className="button-row">
						<button className="button" type="button" onClick={() => openBindEditor(null)}>
							<Plus size={16} />
							Add bind
						</button>
						<button
												...
												<tbody>
													{bind.listeners.map((listener, listenerIndex) => (
														<tr key={`${listener.name}-${listenerIndex}`}>
															...
															<td>
																<span className="badge">{listener.protocol ?? 'HTTP'}</span>
															</td>
															<td>
																{(listener.routes?.length ?? 0) + (listener.tcpRoutes?.length ?? 0)}
															</td>
															...
															<td className="row-actions">
																...
																<Tooltip content="Edit listener">
																	<button
																		...
																		onClick={() =>
																			openListenerEditor(bindIndex, listener, listenerIndex)
																		}
																	>
																		<Pencil size={16} />
																	</button>
																	...
																	<button
																		...
																		onClick={() =>
																			setDeleting({
																				...
																				bindIndex,
																				listenerIndex,
																				label: listenerDisplayName(listener, listenerIndex)
																			})
																		}
																	>
																		<Trash2 size={16} />
					...
					onSave={bind =>
						update.mutate(
							next => {
								...
								const index = next.binds.findIndex(item => item.port === activeBindEditor.port);
	...
	return (
		<Drawer
			...
			footer={requestClose => (
				<ConfigDiffSaveActions
					...
					applyDiff={next => {
						if (!Array.isArray(next.binds)) next.binds = [];
						const index = next.binds.findIndex(item => item.port === props.bind.port);
						const parsed = Number(port);
		...
		>
			...
			<div className="form-grid">
				<Field label="Port" tooltip={props.help.field<TrafficBind>('LocalBind', 'port')}>
		...
		<Drawer
			...
			footer={requestClose => (
				<ConfigDiffSaveActions
					...
					onCancel={requestClose}
					onSave={() =>
						props.onSave(Number(bindIndex), cleanListener(preview), props.editing.listenerIndex)
					}
					applyDiff={next => {
						const bind = next.binds?.[Number(bindIndex)];
						if (!bind) return;
						if (typeof props.editing.listenerIndex === 'number') {
							bind.listeners[props.editing.listenerIndex] = cleanListener(preview);
						} else {
		...
		>
			<div className="form-grid">
				{typeof props.editing.listenerIndex !== 'number' ? (
					<FieldGroup label="Bind" tooltip="Bind port this listener is attached to.">
						<Dropdown
							ariaLabel="Bind"
							value={bindIndex}
				...
				<Field label="Name" tooltip={props.help.field<TrafficListener>('LocalListener', 'name')}>
					<input
						value={listener.name ?? ''}
						onChange={event => setListener({ ...listener, name: event.target.value })}
				...
				<FieldGroup
					label="Protocol"
					tooltip={props.help.field<TrafficListener>('LocalListener', 'protocol')}
				>
					<EnumSelector
						ariaLabel="Protocol"
						value={protocol}
						options={protocols.map(value => ({ value, label: value }))}
						schema={props.help.node(['$defs', 'LocalListener', 'properties', 'protocol'])}
						onChange={value => setListener(makeProtocolListener(listener, value))}
					/>
				</FieldGroup>
			</div>
			<details>
...
function listenerRouteCount(bind: TrafficBind) {
	return bind.listeners.reduce(
		(total, listener) => total + (listener.routes?.length ?? 0) + (listener.tcpRoutes?.length ?? 0),
		0
	);
}
...
function listenerBackendCount(listener: TrafficListener) {
	const http =
		listener.routes?.reduce((total, route) => total + (route.backends?.length ?? 0), 0) ?? 0;
	const tcp =
		listener.tcpRoutes?.reduce((total, route) => total + (route.backends?.length ?? 0), 0) ?? 0;
	return http + tcp;
}
...
function linkedListenerEditor(value: string | null, binds: TrafficBind[]) {
	...
	const listener =
		Number.isInteger(bindIndex) && Number.isInteger(listenerIndex)
			...
			: undefined;
	return listener ? { bindIndex, listenerIndex, listener: structuredClone(listener) } : null;
}
