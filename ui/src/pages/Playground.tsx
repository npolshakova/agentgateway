...
import { extractMcpTools, initializeMcpSession, type McpTool, nextRpcId } from '@/mcp';
import {
	isWildcardModelName,
	modelProviderLabel,
	resolveModelName,
...
type RequestModelOption = { kind: 'model'; config: LlmModel } | { kind: 'virtual' };
...
export function PlaygroundPage() {
	...
	const [storedModel, setStoredModel] = useStoredStringState(storageKeys.model, '');
	...
	const [specificModel, setSpecificModel] = useStoredStringState(storageKeys.specificModel, '');
	...
	const selectedModel = (savedModelExists ? model : '') || modelOptions[0]?.name || '';
	...
	const specificModelSuffix =
		selectedModelConfig && isWildcardModelName(selectedModelConfig.name)
			? wildcardResolvedSuffix(displayRequestModel, selectedModelConfig.name, wildcardPrefix)
			: specificModel;
	...
	const selectedKeyOptionRef = rawVirtualKeys.some(
		...
		: rawVirtualKeys[0]
			...
			: '';
	const savedKey =
		rawVirtualKeys.find((item, index) => virtualKeyStorageRef(item, index) === selectedKeyOptionRef)
			?.key ??
	...
	useEffect(() => {
		if (
			rawVirtualKeys[0]?.key &&
			!rawVirtualKeys.some((item, index) => virtualKeyStorageRef(item, index) === selectedKeyRef)
		) {
	...
	async function send() {
		...
		try {
			...
			setRunSteps([
				{ label: 'Preparing request', state: 'done' },
				...(mcpEnabled ? [{ label: 'Initializing MCP tools', state: 'done' as const }] : []),
			...
			if (mcpEnabled && toolCalls.length > 0) {
				...
				const followUpMessages = [...outboundMessages, assistantMessage, ...toolMessages];
				setRunSteps([
					{ label: 'Preparing request', state: 'done' },
					{ label: 'Initializing MCP tools', state: 'done' },
					{ label: 'Sending chat completion', state: 'done' },
				...
				<StatusBanner
					...
					action={
						hybrid && fileCorsOwned ? (
							...
							<button className="button" type="button" disabled={corsSaving} onClick={applyLlmCors}>
								Apply CORS
							</button>
						)
					}
				...
				<StatusBanner
					...
					action={
						hybrid && fileMcpCorsOwned ? (
							...
							<button className="button" type="button" disabled={corsSaving} onClick={applyMcpCors}>
								Apply MCP CORS
							</button>
						)
					}
				...
				<Panel className="playground-config-panel">
					...
					>
						...
						<Field label="System prompt">
							<textarea rows={4} value={system} onChange={event => setSystem(event.target.value)} />
						</Field>
					</details>
				</Panel>
...
function storedApiKeyMode(): 'saved' | 'raw' {
	return localStorage.getItem(storageKeys.apiKeyMode) === 'raw' ? 'raw' : 'saved';
}
...
function playgroundRequestModel(
	...
	if (option?.kind !== 'model') return resolveModelName(undefined, specificModel, providers);
...
function ChatMessageView(props: { message: ChatMessage }) {
	...
	const Icon = message.role === 'assistant' ? Bot : message.role === 'tool' ? Braces : User;
...
function MessageMetaChips(props: { meta: MessageMeta }) {
	...
	const chips = maybeChips.filter((item): item is { key: string; label: string; icon: ReactNode } =>
		Boolean(item)
	);
		...
		<div className="tool-call-summary">
			...
			{props.message.tool_calls?.map(call => (
				<div className="tool-call-row" key={call.id}>
					...
					<small>{summarizeValue(parseToolArguments(call.function?.arguments))}</small>
				</div>
			))}
		</div>
	);
...
function extractToolCalls(response: unknown): ToolCall[] {
	...
	return calls.filter((call): call is ToolCall =>
		Boolean(call && typeof call === 'object' && typeof (call as { id?: unknown }).id === 'string')
	);
}
...
async function executeToolCalls(
	...
	let activeSessionId =
		sessionId ||
		(await initializeMcpSession(baseUrl, 'agentgateway-ui-llm-playground', sessionId, bearerToken));
	setSessionId(activeSessionId);
	for (const call of calls) {
		const functionName = call.function?.name ?? '';
		const tool = tools.find(
			candidate => candidate.functionName === functionName || candidate.name === functionName
		);
...
function summarizeValue(value: unknown) {
	...
	return truncateOneLine(typeof value === 'string' ? value : JSON.stringify(value), 160);
}
