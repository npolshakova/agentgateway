...
export function ModelMatchesEditor(props: {
	...
	function updateMatch(index: number, match: ModelMatch) {
		props.onChange(matches.map((item, itemIndex) => (itemIndex === index ? match : item)));
	}
		...
		>
			...
			<div className="button-row">
				<button
					...
					type="button"
					onClick={() =>
						props.onChange([...matches, { headers: [{ name: '', value: { exact: '' } }] }])
					}
				>
					<Plus size={16} />
					Add match
...
export function normalizeMatches(matches: LlmModel['matches'] | null | undefined) {
	const next = (matches ?? [])
		.map(match => ({
			...match,
...
function MatchCard(props: {
	...
	function updateHeader(index: number, header: HeaderMatch) {
		props.onChange({
			...props.match,
			headers: headers.map((item, itemIndex) => (itemIndex === index ? header : item))
		});
	}
...
function HeaderMatchRow(props: {
	...
	const mode = value && typeof value === 'object' && 'regex' in value ? 'regex' : 'exact';
	const text =
