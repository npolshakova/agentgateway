			...
			<Panel>
				{mode.isLoading ? (
					...
					<StatusBanner state="warn" title="Readonly policies unavailable">
						Top-level runtime policies are only available when the gateway is running from XDS
						config.
					</StatusBanner>
				) : !policies.length ? (
					<EmptyState
							...
							<tbody>
								{policies.map(policy => (
									<tr key={policy.key}>
										...
										<td>
											<span className="badge">{policyTypeLabel(policy.policy)}</span>
										</td>
...
function policyName(policy: TargetedPolicy) {
	return policy.name ? `${policy.name.namespace}/${policy.name.name}` : policy.key;
}
