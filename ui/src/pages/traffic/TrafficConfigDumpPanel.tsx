...
export function TrafficDumpListenersView(props: {
	...
	const inventory = props.dump ? buildTrafficInventory(props.dump) : null;
	const [selectedListenerKey, setSelectedListenerKey] = useStickyQueryParam('listener');
	...
	return (
		<Panel>
							...
							<section className="traffic-bind readonly" key={bind.key}>
								<div className="traffic-bind-header">
									<div>
										...
										<p>
											{listeners.length} listeners · {listenerRouteCount(bind)} routes
										</p>
									</div>
								...
								{listeners.length ? (
									<RuntimeListenerTable
										...
										onSelect={listener => setSelectedListenerKey(listener.key)}
									/>
								) : (
									<EmptyState
			...
			{selectedListener ? (
				<ListenerDumpDrawer row={selectedListener} onClose={() => setSelectedListenerKey(null)} />
			) : null}
		</Panel>
	);
}
...
export function TrafficDumpRoutesView(props: {
	...
	const selectedRoute = inventory?.routes.find(row => row.route.key === selectedRouteKey);
	return (
		<Panel>
			{props.isLoading ? (
					...
					<table>
						...
						<tbody>
							{inventory.routes.map(row => {
								...
								const backendPolicies = backends.flatMap(backend => backend.inlinePolicies ?? []);
								return (
			...
			<table>
				...
				<tbody>
					{props.rows.map(({ listener }) => (
						<tr key={listener.key}>
							<td className="strong">{listener.listenerName || listener.key}</td>
...
function ListenerDumpDrawer(props: { row: DumpListenerRow; onClose: () => void }) {
	...
	return (
		<Drawer title={listener.listenerName || listener.key} onClose={props.onClose}>
...
function RouteDumpDrawer(props: {
	...
	const backendPolicies = backends.flatMap(backend => backend.inlinePolicies ?? []);
	const resolvedBackends = resolveRouteBackends(backends, props.backends, props.services);
	return (
...
export function buildTrafficInventory(dump: AdminConfigDump) {
	...
	addRouteDump(routes, routeKeys, dump.routes?.routeGroups, 'HTTP', 'route group');
	return {
		listeners,
		routes,
...
export function hasTrafficInventory(inventory: ReturnType<typeof buildTrafficInventory>) {
	return (
...
function listenerRouteObjectCount(listener: DumpListener) {
	return Object.keys(listener.routes ?? {}).length + Object.keys(listener.tcpRoutes ?? {}).length;
}
...
function resolveRouteBackends(
	...
	for (const service of availableServices) {
		servicesByName.set(normalizeBackendName(`${service.namespace}/${service.hostname}`), service);
	}
	return routeBackends
		...
		.filter((backend): backend is BackendWithPolicies | Service => Boolean(backend));
}
...
function routeServiceReferenceName(value: unknown) {
	...
	const namespace = typeof record.namespace === 'string' ? record.namespace : 'default';
...
function hostnamesSummary(hostnames: string[] | undefined) {
	...
	return (
		hostnames.slice(0, 2).join(', ') + (hostnames.length > 2 ? ` +${hostnames.length - 2}` : '')
	);
}
