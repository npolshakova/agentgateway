...
type StringKeys<T> = [KnownStringKeys<T>] extends [never] ? string : KnownStringKeys<T>;
type PropertyValue<T, K extends string> = K extends keyof T ? T[K] : unknown;
export type DeepPath<T> = T extends unknown ? DeepPathOne<NonNullable<T>> : never;
type DeepPathOne<T> = T extends Primitive
	...
	: T extends readonly (infer Item)[]
		...
		: T extends object
			? {
					[K in StringKeys<T>]: NonNullable<PropertyValue<T, K>> extends Primitive
						? K
...
export type SchemaHelp = {
	...
	field<T>(defName: string, propertyPath: DeepPath<T>, fallback?: string): string | undefined;
...
function propertyNode(schema: JsonObject | null, defName: string, propertyPath: string[]) {
