...
import type { LocalRateLimitConfig, SchemaNode } from '@/policies/types';
...
export function policySummary(
	...
	if (key === 'authorization') {
		...
		const grouped =
			authorization.rules && !Array.isArray(authorization.rules) ? authorization.rules : {};
		const ordered = Array.isArray(authorization.rules) ? authorization.rules : [];
		const allow =
			(grouped.allow?.length ?? 0) +
			ordered.filter(
				rule =>
					typeof rule === 'string' || Boolean(rule && typeof rule === 'object' && 'allow' in rule)
			).length;
		const deny =
			(grouped.deny?.length ?? 0) +
			ordered.filter(rule => Boolean(rule && typeof rule === 'object' && 'deny' in rule)).length;
		const require =
			(grouped.require?.length ?? 0) +
			ordered.filter(rule => Boolean(rule && typeof rule === 'object' && 'require' in rule)).length;
...
export function titleFromKey(key: string) {
	...
	return title.replace(/\b(Api|Jwt|Oidc|Cors|Csrf|A2a|Ai|Mcp|Http|Tcp|Tls|Url)\b/g, word => {
			...
			typeof oauth.host === 'string'
				...
				: typeof oauth.backend === 'string'
					...
					: isRecord(oauth.service) && typeof oauth.service.name === 'string'
						? oauth.service.name
						: '';
		return target ? `OAuth token exchange to ${target}` : 'OAuth token exchange';
	}
