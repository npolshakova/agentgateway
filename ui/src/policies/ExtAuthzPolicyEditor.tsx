...
import type { ExtAuthzDraft } from '@/policies/types';
...
export function ExtAuthzPolicyEditor(props: {
	...
	const [protocolMode, setProtocolMode] = useState<ProtocolMode>(
		props.extAuthz?.protocol && 'http' in props.extAuthz.protocol ? 'http' : 'grpc'
	);
					...
					<div className="form-grid">
						...
						>
							<input
								...
								onChange={event =>
									setMaxRequestBytes(event.target.value === '' ? '' : Number(event.target.value))
								}
								placeholder="8192"
							/>
						</Field>
