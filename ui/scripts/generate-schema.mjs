...
mkdirSync(join(uiDir, 'src/generated'), { recursive: true });
mkdirSync(join(uiDir, 'public'), { recursive: true });

const sanitizedConfigSchemaPath = join(uiDir, '.config-schema-for-types.json');
const configSchema = JSON.parse(readFileSync(join(rootDir, 'schema/config.json'), 'utf8'));
writeFileSync(sanitizedConfigSchemaPath, JSON.stringify(sanitizeSchemaForTypes(configSchema)));
const sanitizedAdminSchemaPath = join(uiDir, '.admin-schema-for-types.json');
const adminSchema = JSON.parse(readFileSync(join(rootDir, 'schema/admin.json'), 'utf8'));
writeFileSync(sanitizedAdminSchemaPath, JSON.stringify(sanitizeSchemaForTypes(adminSchema)));

try {
	await Promise.all([
		writeTypes(join(rootDir, 'schema/cel.json'), join(uiDir, 'src/cel.d.ts')),
		writeTypes(sanitizedConfigSchemaPath, join(uiDir, 'src/gateway-config.d.ts')),
...
function sanitizeSchemaForTypes(value) {
	if (Array.isArray(value)) {
		return value.filter(item => !isImpossibleSchema(item)).map(sanitizeSchemaForTypes);
	}
	...
	for (const [key, child] of Object.entries(value)) {
		...
		if ((key === 'oneOf' || key === 'anyOf' || key === 'allOf') && Array.isArray(sanitized)) {
...
function isImpossibleSchema(value) {
	return Boolean(
		value && typeof value === 'object' && Array.isArray(value.enum) && value.enum.length === 0
	);
}
